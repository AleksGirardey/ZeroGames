...
>>>>>>> Stashed changes
        ...
        for ( int i = 1; i<137; i++)
        {
            ...
            {
                points[i] = new Vector3 ( (i*5) , (i * 50) / (Mathf.Pow((i * 5) / (VitesseMaxDefault * Mathf.Sqrt(AccelerationDefault / VitesseMaxDefault)), (2f / 3f))) , 0);
            ...
            else 
            {
                ...
                + ((4 * Mathf.Pow(VitesseMaxDefault, 2)) / Mathf.Pow(distanceRaccourci, 2)))) / (2 * (Mathf.Pow(VitesseMaxDefault, 2) / Mathf.Pow(distanceRaccourci, 2)))) , 1);
